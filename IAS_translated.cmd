...
echo:
call :_color %_Green% "在键盘[1,0]中输入菜单选项："
choice /C:10 /N
if !errorlevel!==2 rem
...
echo:
if not exist "%IDMan%" (
call :_color %Red% "IDM[Internet下载管理器]未安装。"
...
call :_color %Red% "无法连接internetdownloadmanager.com，正在中止。。。"
goto done
)
call :_color %Gray% "internetdownloadmanager.com的Ping命令失败"
echo:
)
...
echo:
call :_color %Gray% "如果出现假串行屏幕，请使用冻结试用选项。"
) else (
call :_color %Green% "IDM 30天试用期成功冷冻终身。"
echo:
call :_color %Gray% "如果IDM显示注册弹出窗口，请重新安装IDM。"
)
